...
- `nucleo_list.html` – Use como template único de listagem. Envolva filtros e listagens dentro de cards. O formulário de busca deve virar `<form class="space-y-4">` com `_forms/field.html` para `form.q` e área de ações no padrão com botões apropriados. Garanta que a grid utilize `card-grid` com espaçamento consistente e que a paginação permaneça dentro da `card-body`.
