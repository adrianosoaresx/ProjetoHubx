(function () {
  const form = document.querySelector('[data-eventos-search-form]');
  if (!form) {
    return;
  }

  const input = form.querySelector('[data-eventos-search-input]');
  const feedback = document.querySelector('[data-eventos-search-feedback]');
  const sections = Array.from(document.querySelectorAll('[data-eventos-search-section]'));
  ...
  const parseTotal = (section) => {
    const raw = section.dataset.eventosSectionTotal;
    if (!raw) {
      return 0;
    }
