...
def test_notificar_responsavel_autoretry():
    ...
    avaliacao = AvaliacaoEmpresa.objects.create(empresa=empresa, usuario=empresa.usuario, nota=5)

    with patch("empresas.tasks.enviar_para_usuario", side_effect=Exception("boom")):
        with patch.object(notificar_responsavel, "retry", wraps=notificar_responsavel.retry) as retry_mock:
