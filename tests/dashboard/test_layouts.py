...
def test_layout_crud(client, admin_user):
    ...
    resp = client.post(
        reverse("dashboard:layout-create"), {"nome": "Meu", "publico": False, "layout_json": json.dumps(["a"])}
    )
...
def test_layout_list_includes_public(client, admin_user, cliente_user):
    own_layout = DashboardLayout.objects.create(user=cliente_user, nome="Own", layout_json="[]")
    public_layout = DashboardLayout.objects.create(user=admin_user, nome="Pub", layout_json="[]", publico=True)
    private_layout = DashboardLayout.objects.create(user=admin_user, nome="Priv", layout_json="[]", publico=False)
    client.force_login(cliente_user)
...
def test_comparativo_endpoint(client, admin_user):
    ...
    User.objects.create_user(
        username="admin2", email="a2@example.com", password="pass", user_type=UserType.ADMIN, organizacao=org2
    )
    client.force_login(admin_user)
...
def test_layout_audit(client, admin_user):
    client.force_login(admin_user)
    resp = client.post(
        reverse("dashboard:layout-create"), {"nome": "L1", "publico": False, "layout_json": json.dumps(["a"])}
    )
    assert resp.status_code == 302
