...
def test_parceria_create_gera_log(api_client: APIClient) -> None:
    ...
    url = reverse("eventos_api:parceria-list")
    data = {
        "evento": evento.pk,
        "empresa": empresa.pk,
        "cnpj": CNPJ().generate(),
...
def test_parceria_delete_gera_log(api_client: APIClient) -> None:
    ...
    api_client.force_authenticate(user)
    url = reverse("eventos_api:parceria-detail", args=[parceria.pk])
...
def test_material_delete_gera_log(api_client: APIClient) -> None:
    ...
    api_client.force_authenticate(user)
    url = reverse("eventos_api:material-detail", args=[material.pk])
    api_client.delete(url)
    assert EventoLog.objects.filter(
        evento=evento,
        usuario=user,
...
def test_briefing_delete_gera_log(api_client: APIClient) -> None:
    ...
    api_client.force_authenticate(user)
    url = reverse("eventos_api:briefing-detail", args=[briefing.pk])
    api_client.delete(url)
    assert EventoLog.objects.filter(
        evento=evento,
        usuario=user,
...
def test_evento_update_gera_log(api_client: APIClient) -> None:
    ...
    api_client.force_authenticate(user)
    url = reverse("eventos_api:evento-detail", args=[evento.pk])
...
def test_evento_list_performance(api_client: APIClient) -> None:
    ...
    api_client.force_authenticate(user)
    url = reverse("eventos_api:evento-list")
    durations = []
    for _ in range(20):
