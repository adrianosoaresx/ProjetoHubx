...
def test_evento_inscritos_pdf_view_returns_pdf(
    ...
    monkeypatch.setattr(eventos_views, "HTML", DummyHTML)
    ...
    assert captured.get("write_pdf_called") is True

    html = captured.get("string", "")
    assert html
    positions = [html.index(nome) for nome in nomes_esperados]
